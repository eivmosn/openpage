...
export function createFieldComponent(options: CreateFieldComponentOptions) {
  ...
  const componentEvent = options.componentEvent === undefined ? 'onchange' : options.componentEvent
  ...
  return defineComponent({
    ...
    setup(rawProps, { slots }) {
      ...
      async function handleUpdate(...args: unknown[]): Promise<void> {
        ...
        if (componentEvent) {
          await props.emitComponentEvent(componentEvent, nextValue)
        }
      }
      ...
      function resolveComponentProps(): Record<string, unknown> {
        ...
        if (placeholderProp && props.component.props.placeholder !== undefined) {
          componentProps[placeholderProp] = props.component.props.placeholder
        }
